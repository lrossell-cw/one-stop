...
- Hosting — evaluate Canvas (CoreWeave's internal app-hosting platform) as the deploy target for workstreams. Recommended fit: built for React/Express + managed Postgres, handles auth/secrets/networking, has an AI-scaffolding skills library. Not started — needs Glean / `go/app-hosting-advisor` verification that Canvas fits DC-floor requirements (network reach to the floor, offline/local-network needs for scanner-gun workflows) before treating it as settled.
- Mobile integration — get workstreams (rack-lookup and walkthrough especially, given camera-scan requires HTTPS and silently fails over plain LAN HTTP per rack-lookup's README) properly testable/usable on a phone, not just LAN-IP workarounds during dev. **Follows Hosting**, deliberately sequenced after it: current plan is to host on Canvas in the CoreWeave workspace, but that's not yet confirmed (see Hosting above) — real HTTPS mobile access depends on where this actually deploys, so this shouldn't be scoped in detail until Hosting resolves. Prioritized early relative to other backlog items specifically because Leo wants to test functionality on-device sooner rather than later. Discuss further once this workstream starts.
